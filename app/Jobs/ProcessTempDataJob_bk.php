...
class ProcessTempDataJob extends Job implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        $holidayDutyRequisitions = HolidayDutyRequisition::where('date_from', '>=', $startDate)
            ...
            ->get()
            ->keyBy('holiday_duty_requisition_details.duty_date');

            Log::warning('holiday_duty_requisitions:', [
        ...
        $totalLeaveAchieved =  EmployeeLeaveAchieveLog::whereIn('employee_attendance_id', $attRecordIds)->selectRaw('SUM(leave_count),employee_leave_balance_id')
    ...
    if ($attRecordIds->isNotEmpty()) 
        {
            ...
            foreach ($totalLeaveAchieved as $item) {
                $leaveBalance = EmployeeAttendance::where('id', $item->employee_leave_balance_id)->first();
                $leaveBalance->leave_count -= $item->leave_count;
                $leaveBalance->save();
            }
            ...
            */
            ...
            foreach ($dates as $date) {
                ...
                if ($holidayDutyRequisition_all) {
                    $holidayDutyRequisition = $holidayDutyRequisition_all
                ...
                if (
                    ...
                    if ($LeaveApplicationDetail && $LeaveApplicationDetail->where('leave_form_type', 1)->where('first_second_half', 4)->contains('leave_date', $date)) {
                ...
                if (
                    ...
                    //$has_1st_halfday_leave = $LeaveApplicationDetail->where('leave_form_type',1)->where('first_second_half',1)->contains('leave_date', $date)->exists();
                    if ($LeaveApplicationDetail && $LeaveApplicationDetail->where('leave_form_type', 1)->where('first_second_half', 6)->contains('leave_date', $date)) {
