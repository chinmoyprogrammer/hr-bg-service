...
return new class extends Migration {
    public function up(): void
    {
        Schema::create('subsections', function (Blueprint $table) {
            ...
            $table->string('subsection_name', 150);
            $table->string('subsection_name_bn', 150)->nullable();
            $table->string('subsection_short_name', 30);
            // Override to users.id as per instruction
            $table->unsignedBigInteger('subsection_head_id')->nullable();
            ...
            $table->unique('subsection_name');
            $table->unique('subsection_short_name');

            $table->foreign('company_id')->references('id')->on('companies')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('branch_id')->references('id')->on('branches')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('department_id')->references('id')->on('departments')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('section_id')->references('id')->on('sections')->onUpdate('cascade')->onDelete('cascade');
            $table->foreign('subsection_head_id')->references('id')->on('users')->onUpdate('cascade')->onDelete('set null');
